...
<?php if ( is_singular( array( 'post' ) ) && ( !has_post_thumbnail( $post->ID ) ) ){ ?>
	<div class="headerTitle" style="background:url(<?php echo get_stylesheet_directory_uri(); ?>/images/inner-banner-bg.jpg) no-repeat center center;background-size:cover;">
...
<?php if ( is_singular( array( 'page' ) ) && has_post_thumbnail( $post->ID ) ){ ?>
	<div class="headerTitle" style="background:url(<?php echo get_the_post_thumbnail_url( $post_id, 'full' ); ?>) no-repeat center center;background-size:cover;">
		<div class="blackBg"></div>
		<div class="container"><h1><?php the_title(); ?> <small><?php echo do_shortcode('[acf field="header_sub_title"]'); ?></small></h1></div>
	</div>	
<?php }	?>
