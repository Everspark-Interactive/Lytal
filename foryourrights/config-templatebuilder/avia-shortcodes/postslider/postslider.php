		...
		public function html()
		{
			...
			$output .= 		"<div class='avia-content-slider-inner'>";
				...
				foreach ($this->entries->posts as $entry)
				{
					...
					$output .= $thumbnail ? "<a href='{$link}' data-rel='slide-".avia_post_slider::$slide."' class='tt slide-image' title=''>{$thumbnail}</a>" : "";
					...
					$output .= "<div class='slide-content test'><span class='pub'>". get_the_date( 'd M Y', $the_id )."</span>";
					// Get the author's name and author archive URL
$author_id = get_post_field('post_author', $the_id);
$author_name = get_the_author_meta('display_name', $author_id);
$author_url = get_author_posts_url($author_id);

if (!empty($author_name)) {
    $output .= "<span class='author'> by <a href='{$author_url}'>{$author_name}</a></span>"; // Output the author's name as a link
}
